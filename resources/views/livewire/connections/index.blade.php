<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold">{{ __('connections.title') }}</h1>
    ...
    @if ($own->isEmpty())
        <div class="rounded-md border border-dashed border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-8 text-center text-sm text-zinc-500 dark:text-zinc-400">
            {{ __('connections.empty') }}
            <a href="{{ route('connections.create') }}" wire:navigate class="text-zinc-900 dark:text-zinc-100 underline">{{ __('connections.empty_cta') }}</a>.
        </div>
    @else
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden flex flex-col">
                    ...
                    <div class="p-4 flex-1 flex flex-col gap-3">
                        ...
                        <div class="text-xs text-zinc-500 dark:text-zinc-400">
                            @if ($c->last_used_at)
                                {{ __('connections.last_used', ['ago' => $c->last_used_at->diffForHumans()]) }}
                            @else
                                {{ __('connections.never_used') }}
                            @endif
                            @if ($c->ssl) · SSL @endif
                        </div>
                        ...
                        <div class="mt-auto flex items-center gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                            @if ($activeId === $c->id)
                                <span class="inline-flex items-center gap-1 text-xs font-medium text-emerald-700">
                                    <span class="size-1.5 rounded-full bg-emerald-500"></span> {{ __('connections.active') }}
                                </span>
                                <button wire:click="deactivate"
                                    class="text-xs text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-zinc-100">{{ __('connections.disconnect') }}</button>
                            @else
                                <button wire:click="activate({{ $c->id }})"
                                    class="text-xs font-medium text-zinc-900 dark:text-zinc-100 hover:underline">{{ __('connections.use_this') }}</button>
                            @endif
                            <span class="ml-auto"></span>
                            <a href="{{ route('connections.edit', $c) }}" wire:navigate class="text-xs text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-zinc-100">{{ __('connections.edit') }}</a>
                            <button wire:click="delete({{ $c->id }})"
                                wire:confirm="{{ __('connections.delete_confirm', ['name' => $c->name]) }}"
                                class="text-xs text-rose-600 hover:text-rose-700">{{ __('connections.delete') }}</button>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    @if ($shared->isNotEmpty())
        <div class="space-y-3 pt-4">
            <h2 class="text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">{{ __('connections.shared_title') }} <span class="font-normal normal-case text-zinc-400 dark:text-zinc-500">{{ __('connections.shared_readonly') }}</span></h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($shared as $c)
                    <div class="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden">
                        ...
                        <div class="p-4 space-y-1">
                            ...
                            <div class="text-xs text-zinc-500 dark:text-zinc-400 truncate">{{ __('connections.shared_owned_by', ['email' => $c->user->email]) }}</div>
                            <div class="text-xs text-zinc-500 dark:text-zinc-400 font-mono truncate">
                                {{ $c->driver === 'sqlite' ? $c->database : ($c->username . '@' . $c->host . ($c->port ? ':' . $c->port : '') . ($c->database ? '/' . $c->database : '')) }}
                            </div>
                        </div>
