<div class="h-full flex flex-col"
    ...
    {{-- Toolbar --}}
    <div class="shrink-0 px-4 py-2 border-b border-zinc-200 bg-white flex items-center gap-3 text-xs text-zinc-600 flex-wrap">
        <span class="font-medium text-zinc-800">{{ $currentLabel }}</span>
        <span class="text-zinc-300">/</span>

        <label class="flex items-center gap-1">
            <span class="text-zinc-500">Database</span>
            <select wire:model="database" class="text-xs border border-zinc-300 rounded px-1.5 py-1">
        ...
        <label class="flex items-center gap-1.5 text-zinc-700">
            <input type="checkbox" wire:model="compact" class="rounded border-zinc-300" />
            Compact (PK + FK only)
        </label>

        <label class="flex items-center gap-1">
            <span class="text-zinc-500">Layout</span>
            <select wire:model.live="layout" class="text-xs border border-zinc-300 rounded px-1.5 py-1">
        ...
        @if ($tableCount > 0)
            <div class="ml-auto flex items-center gap-2">
                <input type="search" x-model.debounce.200ms="search" placeholder="Highlight table…"
                    class="w-40 rounded border border-zinc-300 px-2 py-1 text-xs" />
                <span class="text-zinc-500">
                    <span class="font-mono text-zinc-700">{{ $tableCount }}</span> entities ·
                    <span class="font-mono text-zinc-700">{{ $relationshipCount }}</span> relationships
                ...
                <button type="button" @click="fit()"
                    ...
                    class="inline-flex items-center justify-center size-7 rounded border border-zinc-300 hover:bg-zinc-50">
                ...
                <button type="button" @click="downloadPng()"
                    class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded border border-zinc-300 hover:bg-zinc-50 text-xs">
                    <svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    {{-- Body --}}
    <div class="flex-1 min-h-0 flex bg-zinc-50">
        <div class="flex-1 min-w-0 min-h-0 relative">
            ...
            @elseif ($tableCount === 0)
                <div class="absolute inset-0 flex flex-col items-center justify-center text-center text-zinc-500 text-sm">
                    ...
                    Pick a database and click <span class="font-medium text-zinc-700 mx-1">Generate</span> to render the schema diagram.
                    <p class="mt-2 text-xs text-zinc-400 max-w-md">Tip : enable <em>Compact</em> on huge schemas (200+ tables) to keep PK + FK columns only. Click a node to highlight its relationships, drag nodes to rearrange.</p>
                </div>
            @else
                <div x-ref="diagram"
        ...
        <aside x-show="selected" x-cloak x-transition
            class="w-80 shrink-0 border-l border-zinc-200 bg-white overflow-y-auto">
            <template x-if="selected">
                <div class="p-4 space-y-3">
                    <div class="flex items-center justify-between">
                        <h3 class="text-sm font-semibold text-zinc-900" x-text="selected.label"></h3>
                        <button type="button" @click="selected = null"
                            class="text-zinc-400 hover:text-zinc-700 text-lg leading-none">&times;</button>
                    </div>
                    <p class="text-[10px] font-mono text-zinc-500" x-text="selected.qualified"></p>
                    ...
                    <ul class="space-y-px text-xs font-mono">
                        <template x-for="col in (selected.columns || [])" :key="col.name">
                            <li class="flex items-center gap-1.5 px-1.5 py-1 rounded hover:bg-zinc-50">
                                <span class="truncate flex-1" x-text="col.name"></span>
                                <span class="text-zinc-400" x-text="col.type"></span>
