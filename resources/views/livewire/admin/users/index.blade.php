<div class="space-y-6">
    ...
    <div>
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search by name or email…"
            class="w-full max-w-sm rounded-md border border-zinc-300 px-3 py-2 text-sm" />
    </div>
    ...
    <div class="overflow-x-auto bg-white border border-zinc-200 rounded-lg">
        <table class="w-full text-sm">
            ...
            <tbody>
                @foreach ($users as $u)
                    <tr class="border-b border-zinc-100 last:border-0" wire:key="user-{{ $u->id }}">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 p-4" wire:click.self="$set('showCreate', false)">
            <div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 space-y-4">
                ...
                <form wire:submit="createUser" class="space-y-3">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-zinc-700 mb-1">Initial password (leave empty to auto-generate)</label>
