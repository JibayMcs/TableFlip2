<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <span class="text-xs text-zinc-500">Files expire after {{ config('tableflip.exports.retention_days', 7) }} days.</span>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    @foreach ($exports as $export)
                        <tr class="border-b border-zinc-100 last:border-0" wire:key="export-{{ $export->id }}">
                            <td class="px-4 py-2 font-mono text-xs">{{ $export->file_name }}</td>
                            <td class="px-4 py-2">
                                <span class="font-mono text-xs uppercase rounded bg-zinc-100 px-1.5 py-0.5">{{ $export->format }}</span>
                            </td>
                            ...
                            <td class="px-4 py-2 text-right font-mono text-xs text-zinc-600">
                            ...
                            <td class="px-4 py-2 text-xs text-zinc-500">{{ $export->created_at->diffForHumans() }}</td>
                            <td class="px-4 py-2 text-right space-x-2 whitespace-nowrap">
                                @if ($export->isCompleted() && $export->download_url && ! $export->isExpired())
                                    <a href="{{ $export->download_url }}"
                                        class="text-xs text-zinc-900 hover:underline font-medium">Download</a>
                                @endif
