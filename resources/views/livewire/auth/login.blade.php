<div class="max-w-md mx-auto">
    <div class="bg-white border border-zinc-200 rounded-lg shadow-sm">
        @if ($breezeEnabled && $directEnabled)
            <div class="flex border-b border-zinc-200" role="tablist">
                <button
                    type="button"
                    wire:click="$set('mode', 'account')"
                    class="flex-1 px-4 py-3 text-sm font-medium transition-colors {{ $mode === 'account' ? 'text-zinc-900 border-b-2 border-zinc-900 -mb-px' : 'text-zinc-500 hover:text-zinc-700' }}"
                >
                    Account
                </button>
                <button
                    ...
                    class="flex-1 px-4 py-3 text-sm font-medium transition-colors {{ $mode === 'direct' ? 'text-zinc-900 border-b-2 border-zinc-900 -mb-px' : 'text-zinc-500 hover:text-zinc-700' }}"
                >
                    Direct database
                </button>
            </div>
            ...
            @if ($mode === 'direct' && $directEnabled)
                <form wire:submit="loginDirect" class="space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div class="col-span-1">
                            ...
                            <select
                                ...
                                class="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm focus:border-zinc-900 focus:outline-none focus:ring-1 focus:ring-zinc-900 {{ $driverLocked ? 'bg-zinc-50 text-zinc-500' : '' }}"
                            >
                        ...
                        <div class="col-span-2">
                            ...
                            <input
                                ...
                                class="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm focus:border-zinc-900 focus:outline-none focus:ring-1 focus:ring-zinc-900 {{ ($hostLocked || $driver === 'sqlite') ? 'bg-zinc-50 text-zinc-500' : '' }}"
                            />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div class="col-span-1">
                            ...
                            <input
                                ...
                                placeholder="auto"
                                class="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm focus:border-zinc-900 focus:outline-none focus:ring-1 focus:ring-zinc-900 {{ $driver === 'sqlite' ? 'bg-zinc-50 text-zinc-500' : '' }}"
                            />
                        </div>
                    ...
                    @if ($driver !== 'sqlite')
                        ...
                        <div>
                            <label for="directPassword" class="block text-sm font-medium text-zinc-700 mb-1">Password</label>
                            <input
                                id="directPassword"
                                type="password"
