<div class="h-full flex">
    ...
    <section class="flex-1 min-w-0 flex flex-col">
        ...
        <div class="shrink-0 px-4 py-2 border-b border-zinc-200 bg-white flex items-center gap-3 text-xs text-zinc-600">
            <span class="font-medium text-zinc-800">{{ $currentLabel }}</span>
            ...
            <span class="text-zinc-300">·</span>
            <span class="text-zinc-500">dialect: <code class="text-zinc-700">{{ $dialect }}</code></span>

            <div class="ml-auto flex items-center gap-2">
                <kbd class="px-1.5 py-0.5 text-[10px] bg-zinc-100 border border-zinc-200 rounded">⌘/Ctrl + Enter</kbd>
                <span class="text-zinc-400">to execute</span>
            </div>
        </div>

        {{-- Tabs --}}
        <div class="shrink-0 flex items-center gap-px bg-zinc-100 border-b border-zinc-200 overflow-x-auto">
            @foreach ($tabs as $tab)
                <div class="group flex items-center gap-1 px-3 py-1.5 text-xs cursor-pointer whitespace-nowrap {{ $tab['id'] === $activeTabId ? 'bg-white text-zinc-900 font-medium border-t-2 border-zinc-900 -mb-px' : 'text-zinc-600 hover:bg-zinc-50 border-t-2 border-transparent' }}"
                    ...
                    <button type="button" wire:click.stop="closeTab('{{ $tab['id'] }}')"
                        class="opacity-0 group-hover:opacity-100 text-zinc-400 hover:text-rose-600 leading-none px-1">×</button>
                </div>
            @endforeach
        ...
        {{-- Result --}}
        <div class="shrink-0 max-h-[40vh] overflow-auto border-t border-zinc-200 bg-white">
            @if ($lastResult === null)
            ...
            @else
                <div class="px-3 py-1.5 text-xs text-zinc-500 border-b border-zinc-100 flex items-center gap-3 bg-zinc-50">
                    <span><span class="font-mono font-medium text-zinc-700">{{ number_format(count($lastResult['rows'])) }}</span> rows</span>
                    <span class="text-zinc-300">·</span>
                    <span>{{ number_format($lastResult['durationMs'], 1) }} ms</span>
                </div>
                @if (count($lastResult['rows']) === 0)
                    <div class="px-4 py-6 text-center text-xs text-zinc-400">Empty result set.</div>
                @else
                    <table class="w-full text-xs border-collapse">
                        <thead class="text-left text-[10px] uppercase text-zinc-500 border-b border-zinc-200 bg-zinc-50">
                            <tr>
    ...
    <aside class="w-72 shrink-0 border-l border-zinc-200 bg-white overflow-y-auto flex flex-col">
        <div class="sticky top-0 z-10 bg-white border-b border-zinc-100 p-3">
            <div class="flex items-center justify-between mb-2">
                <div class="text-xs uppercase tracking-wide text-zinc-500 font-semibold">History</div>
                @if ($history->isNotEmpty())
                    <button type="button" wire:click="clearAllHistory"
                        ...
                        class="text-zinc-400 hover:text-rose-600 p-1 -m-1 rounded">
            ...
            <input type="search" wire:model.live.debounce.300ms="historySearch" placeholder="Search…"
                class="w-full rounded-md border border-zinc-300 px-2.5 py-1.5 text-xs" />
        </div>

        <div class="flex-1 divide-y divide-zinc-100">
            @forelse ($history as $entry)
                <div class="group p-2.5 hover:bg-zinc-50/60" wire:key="hist-{{ $entry->id }}">
                    <div class="flex items-center gap-2 text-[10px] mb-1">
                        ...
                        @endif
                        <span class="text-zinc-500">{{ $entry->executed_at->diffForHumans() }}</span>
                        <span class="ml-auto text-zinc-400 font-mono">{{ $entry->duration_ms }}ms</span>
                        <button type="button" wire:click="deleteHistoryEntry({{ $entry->id }})"
                            x-tooltip.left="Delete this entry"
                            class="inline-flex items-center justify-center size-4 rounded text-zinc-400 hover:text-rose-600 hover:bg-rose-50 transition-colors">
                    ...
                    <button type="button" wire:click="loadFromHistory({{ $entry->id }})"
                        ...
                        <div class="font-mono text-xs text-zinc-700 line-clamp-3 break-all">{{ $entry->sql_text }}</div>
                        @if ($entry->database_name)
                            <div class="mt-1 text-[10px] text-zinc-400 font-mono">{{ $entry->database_name }}</div>
                        @endif
                    </button>
                </div>
            @empty
                <div class="p-4 text-center text-xs text-zinc-400 italic">
                    @if ($historySearch !== '')
